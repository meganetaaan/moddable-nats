...
function publishMove() {
  try {
    ...
    globalThis.nc.publish("lc.s.cmd", JSON.stringify(payload));
  } catch (e) {
...
async function setupNats(controlView) {
  ...
  try {
    ...
    // Subscribe to cart safety transitions and update joystick background
    try {
      const safetySub = await nc.subscribe("cart_safety.evt.transition");
      (async () => {
        for await (const m of safetySub) {
          try {
            const s = m.string();
            try { trace(`Received on [${m.subject}] : '${s}'\n`); } catch {}
            let obj = null;
            try { obj = JSON.parse(s); } catch {}
            if (obj && obj.evt === "state_transition") {
              const state = String(obj.state || "normal");
              try { controlView.behavior.setSafetyState(controlView, state); } catch {}
            }
          } catch {}
        }
      })();
